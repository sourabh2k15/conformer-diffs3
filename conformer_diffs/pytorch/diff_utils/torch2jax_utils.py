...
class Torch2Jax:
  ...
  def key_transform(self, key_transform_fn):
    self.pytorch_sd = {
        key_transform_fn(k): self.pytorch_sd[k] for k in self.pytorch_sd
    }
  ...
  def diff(self):
    jax_key_set = set(self.flattened_jax_model.keys()) - set(self.pytorch_sd.keys())
    pytorch_key_set = set(self.pytorch_sd.keys()) - set(self.flattened_jax_model.keys())
    jax_pytorch_key_intersection = set(self.pytorch_sd.keys()) & set(self.flattened_jax_model.keys())
    print(f'Keys in jax but not in pytorch: {len(jax_key_set)}')
    pprint.pprint(sorted(list(jax_key_set)))

    print(f'Keys in pytorch but not in jax: {len(pytorch_key_set)}')
    pprint.pprint(sorted(list(pytorch_key_set)))

    print(f'Common keys: {len(jax_pytorch_key_intersection)}')

    if len(jax_pytorch_key_intersection) == len(self.pytorch_sd):
      count = 0
      for k in self.pytorch_sd:
